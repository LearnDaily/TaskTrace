...
class TreeNodeHelper {
    ...
    func insertNode(node:TreeNode,pId:String){
        if node.id == pId {
        ...
        for n in node.children {
            insertNode(node, pId: pId)
        }
    }
    
    func findNodeById( nodes:[TreeNode], var id:String)->TreeNode?{
        ...
        for node in nodes {
            ...
            if node.id == id {
                ...
                print("fidn node is: \((node as! TaskModel).title) id:\(id)")
                return node
            }
    ...
    // 从一个node中找到父亲节点
    func findParent(node:TreeNode,pId:String) ->TreeNode?{
            ...
            for n in node.children {
                var target =  findParent(n,pId:pId)
                if target != nil {
                    return target
                }
            }
